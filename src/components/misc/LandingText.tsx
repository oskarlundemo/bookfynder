...
export default function LandingText({title, breadText, number, reverse}: Props) {
    ...
    return (
        <article className="relative md:w-1/2 w-full flex flex-col items-center gap-5">
            ...
            >
                <span className={'h-5 w-5 p-5 font-extrabold text-2xl flex items-center justify-center text-white bg-black rounded-full'}>
                    {number}
                </span>

                <h2 className="text-[clamp(1rem,2vw,2rem)]  font-semibold">
                    {title}
                </h2>
            </div>
